...
const Button: React.FC<ButtonProps> = ({text, visibility, onClick}) => {
    ...
    return (
        ...
        className={`${visibility} relative border-2 text-sm text-slate-700 border-slate-700 font-bold py-2 px-3 rounded 
        hover:bg-slate-800 hover:text-slate-100 duration-300`}>{text}</button>
    );
}
